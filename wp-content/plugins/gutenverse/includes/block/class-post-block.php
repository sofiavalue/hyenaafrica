...
class Post_Block extends Post_Abstract {
	...
	public function get_primary_category( $post_id ) {
		...
		if ( ! empty( $this->attributes['categoryEnabled'] ) && $cat_id ) {
	...
	public function post_meta( $post ) {
		...
		if ( ! empty( $this->attributes['metaEnabled'] ) ) {
			$author_output = null;
			$date_output   = null;

			if ( ! empty( $this->attributes['metaAuthorEnabled'] ) ) {
			...
			if ( ! empty( $this->attributes['metaDateEnabled'] ) ) {
	...
	protected function get_excerpt( $post ) {
		...
		if ( ! empty( $this->attributes['excerptEnabled'] ) ) {
	...
	protected function get_readmore( $post ) {
		...
		if ( ! empty( $this->attributes['readmoreEnabled'] ) ) {
	...
	protected function get_comment_bubble( $post ) {
		...
		if ( ! empty( $this->attributes['commentEnabled'] ) ) {
